...
// Occupancy fact for this kernel; the device-wide target is one resident wave, so it must come
// from the real multiprocessor count rather than a fixed part's SM count.
...
cudaError_t launch_output(const chunk_output_config& cfg) {
    ...
    const std::int64_t target_ctas =
        kCtasPerSm * static_cast<std::int64_t>(device_multiprocessor_count());
