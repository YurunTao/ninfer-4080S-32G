...
// The persistent prefill grid must match the resident capacity of the real device, not a fixed
// part's SM count: an oversubscribed grid turns the persistent loop into extra waves.
inline int prefill_persistent_blocks() noexcept {
    return kPrefillBlocksPerSm * device_multiprocessor_count();
}
