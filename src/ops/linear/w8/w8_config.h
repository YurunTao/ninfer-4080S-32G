...
#ifdef NINFER_SM86
    // sm_86/89 keep the static staging under the 48 KiB per-CTA limit; eight warps with
    // 40/48-token tiles exceed it, so the four-warp shape serves those tiles.
    (TileTokens >= 40 ? 4 : 8),
#else
    8,
#endif
    TileTokens, TileTokens == 8 ? 5 : (TileTokens == 16 ? 4 : (TileTokens == 24 ? 3 : 2)),
...
struct W8LinearSmallTProductionSchedule<W835bMtpProjectionGeometry, ActiveTokens> {
    ...
    static constexpr int kKWarps =
#ifdef NINFER_SM86
        // sm_86/89 keep static staging within 48 KiB: the sixteen-warp split and the
        // eight-warp 40/48-token tiles both exceed it.
        ActiveTokens <= 12 ? 8 : 4;
#else
        ActiveTokens <= 12 ? 16 : 8;
#endif
