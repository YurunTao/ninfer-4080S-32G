...
The implementation targets `sm_89` and is tuned and measured on NVIDIA GeForce RTX 4080 SUPER.
The Qwen3.8-27B target supports DFlash2 speculative decoding with startup-selected draft windows
`1..15` (`--spec dflash2 --draft-tokens K`, companion weights via the artifact converter); the
Qwen3.6-35B-A3B target additionally supports text-only DFlash.
...
- `.ninfer` is the only C++ product artifact; CLI, serving, and inference benchmarks use the public
  Engine. NInfer does not install or export a C++ SDK; `tools/reference` retains the Python
  reference stack used by converter verification.
...
Use the selected Python interpreter explicitly for converter/reference work. On this machine the
torch-capable environment is `/home/raymond/sglang/.venv/bin/python`; the default shell's `python3`
may lack torch/safetensors. The normal build directory is `build-sm89` (CMAKE_CUDA_ARCHITECTURES=89,
CUDA 13.0); product artifacts and conversion reports live under `out/`. Profiler output goes to
`profiles/ncu/`, `profiles/nsys/`, `profiles/bench/`.
