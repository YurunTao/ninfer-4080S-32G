...
int run_nvfp4() {
    ...
    for (int width = 2; width <= 16; ++width) {
        failures += run(width, 1, {}, ops::LinearPolicy::A16Only, 1600U + width);
        failures += run(width, 8, ragged(width, 8), ops::LinearPolicy::A16Only, 1650U + width);
    }
#ifndef NINFER_SM86
    ...
    for (int width = 2; width <= 16; ++width) {
        failures += run(width, 1, {}, ops::LinearPolicy::AllowA4, 1680U + width);
        failures += run(width, 8, ragged(width, 8), ops::LinearPolicy::AllowA4, 1700U + width);
    }
#endif
